function [EEG, report, suffix] = atclv2_ANACRA_SPM_preproc_YY(EEG, param, ~)
tic
spm('defaults', 'eeg');
...
if param.isSpectral == 1
    ...
    alldata_log=[];
    alldata_ph=[];
    if ~exist([param.exportPath filesep subN filesep], 'dir')
        mkdir([param.exportPath filesep subN filesep]);
    end
    ...
    %writecell(idxRows, [param.exportPath param.outFolder filesep 'time_' subN '_' probType '_' trialN '.csv']);
 
    for j=1:length(chanlabels)
        writematrix(alldata_log(:,:,j), [param.exportPath filesep subN filesep 'log_' subN '_' probType '_' trialN '_' chanlabels{j} '.csv']);
        writematrix(alldata_ph(:,:,j), [param.exportPath filesep subN filesep 'ph_' subN '_' probType '_' trialN '_' chanlabels{j} '.csv']);
    end
    toc
